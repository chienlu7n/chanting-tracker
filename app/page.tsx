    ...
    <div style={{ fontFamily: '"rounded-mplus-1c", sans-serif', backgroundImage: 'linear-gradient(rgba(255, 255, 255, 0.2), rgba(255, 255, 255, 0.2)), url("/背景.png")', backgroundSize: 'cover', backgroundPosition: 'center', minHeight: '100vh', padding: '40px 20px', color: '#4a4a4a' }}>
      <div style={{ maxWidth: '500px', margin: '0 auto', backgroundColor: 'rgba(253, 251, 247, 0.9)', borderRadius: '30px', padding: '40px', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', border: '6px solid #a3d977' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={status === 'working'} style={{ width: '100%', padding: '18px', backgroundColor: '#669933', color: '#fff', border: 'none', borderRadius: '50px', fontWeight: 'bold', cursor: 'pointer' }}>{status === 'working' ? '搬運中...' : '📣 紀錄日記！'}</button>
        </form>
        <div style={{ marginTop: '30px', borderTop: '2px dashed #a3d977', paddingTop: '20px' }}>
          <h4 style={{ color: '#5e9438' }}>🕒 最新 5 筆紀錄:</h4>
          {recentRecords.map((r) => (<div key={r.id} style={{ fontSize: '14px', marginBottom: '5px' }}>• {r.user_name} 唱題 {r.duration} 分鐘</div>))}
        </div>
        <button onClick={() => router.push('/stats')} style={{ width: '100%', marginTop: '20px', padding: '15px', backgroundColor: '#ffd54f', border: 'none', borderRadius: '50px', fontWeight: 'bold', cursor: 'pointer' }}>📊 查看團隊目標</button>
